...
export function useUpdatePaymentReceiptStatus() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      // If approved, update the resident's payment status
      if (status === "APPROVED") {
        await updateResidentPaymentStatus(receipt.resident_id, "APPROVED");
      } else if (status === "REJECTED") {
        // Check if all receipts are rejected
        ...
        const hasApproved =
          otherReceipts?.some((r) => r.status === "APPROVED") || false;
        if (!hasApproved) {
          await updateResidentPaymentStatus(receipt.resident_id, "PENDING");
        }
      }
...
// Helper function to update resident payment status
async function updateResidentPaymentStatus(residentId: string, status: string) {
  const { error } = await supabase
    .from("resident_info")
    .update({
  ...
  if (error) {
    console.error("Failed to update resident payment status:", error);
  }
}
