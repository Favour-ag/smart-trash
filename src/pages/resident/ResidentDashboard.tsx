...
const ResidentDashboard = () => {
  ...
  const { data: collections = [], isLoading: loadingCollections } = useCollections('PENDING');
  const { data: stats, isLoading: loadingStats } = useDashboardStats();
  const { data: residentInfo } = useResidentInfo();

  // Find the next collection
  const nextCollection = collections.length > 0 
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Welcome to your waste management dashboard.</p>
      </div>
      ...
      <Card>
        ...
        <CardContent className="p-6">
          {loadingAnnouncements ? (
            ...
            <div className="text-center py-4">No announcements at this time</div>
          ) : (
            <div className="space-y-4">
              {announcements.map((announcement) => (
                <div key={announcement.id} className={`border-l-4 border-tw-${announcement.category === 'alert' ? 'red' : announcement.category === 'info' ? 'blue' : 'green'}-500 pl-4 py-1`}>
                  <p className="font-medium">{announcement.title}</p>
                  <p className="text-sm text-gray-600">{announcement.message}</p>
                  <p className="text-xs text-gray-500 mt-1">
                    Posted on {format(new Date(announcement.posted_at), 'MMM d, yyyy')}
                  </p>
                </div>
              ))}
            </div>
            ...
            {loadingCollections ? (
              ...
              <div className="mt-2 space-y-1">
                <div className="text-2xl font-bold text-tw-green-700">
                  {format(new Date(nextCollection.collection_date), 'EEEE, MMM d')}
                </div>
                <div className="text-sm text-gray-500">
                  {nextCollection.waste_type || 'General Waste'}
                </div>
                <div className="text-sm font-medium text-tw-green-600">
                  {differenceInDays(new Date(nextCollection.collection_date), today) === 0 
                    ? 'Today' 
                    : differenceInDays(new Date(nextCollection.collection_date), today) === 1
                      ? 'Tomorrow'
                      : `${differenceInDays(new Date(nextCollection.collection_date), today)} days from now`}
                </div>
              </div>
            ) : (
              <div className="mt-2">
                <div className="text-lg font-medium">No upcoming collections</div>
        ...
        <Card>
          ...
          <CardContent>
            {loadingCollections ? (
              ...
              <div className="text-center py-2">No recent collection history</div>
            ) : (
              <div className="space-y-2">
                {pastCollections.map(collection => (
                  <div key={collection.id} className="flex justify-between items-center">
                    <div>
                      <div className="font-medium">{collection.waste_type || 'General Waste'}</div>
                      <div className="text-xs text-gray-500">
                        {format(new Date(collection.collection_date), 'MMM d, yyyy')}
                      </div>
                    </div>
                    <span className={`text-xs px-2 py-1 bg-${collection.status === 'COLLECTED' ? 'green' : 'red'}-100 text-${collection.status === 'COLLECTED' ? 'green' : 'red'}-800 rounded-full`}>
                      {collection.status === 'COLLECTED' ? 'Collected' : 'Missed'}
                    </span>
                  </div>
                ))}
              </div>
